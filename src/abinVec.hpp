...
template <typename T>
    void Abin<T>::insertarHijoIzqdoB(Abin<T>::nodo n, const T& e)
    {
        ...
        assert(nodos[n].hizq == NODO_NULO);     //n no tiene ya un hijo izqd.
...
template <typename T>
    void Abin<T>::eliminarHijoIzqdoB(Abin<T>::nodo n)
    {
        ...
        assert(nodos[hizqdo].hizq == NODO_NULO &&   //el hijo izqdo. de n
        ...
        if(hizqdo != numNodos -1)
        {
            ...
            //Actualizar la posición de hijo (izquierdo o derecho)
